...
- `UI/`: CSSフレームワーク本体およびWYSIWYG Editor UIスキン。`UI/adlaire.css`、`UI/base.css`、`UI/grid.css`、`UI/layout.css`、`UI/components.css`、`UI/site.css`、`UI/forms.css`、`UI/content.css`、`UI/wysiwyg.css`、`UI/utilities.css`、`UI/compat-agws.css` を管理する。`UI/compat-agws.css` はAdlaire-Design仕様の既存ベース互換層であり、AGWSを参照正本にする意味ではない。
...
仕様・設計の正本は `Docs/Master_Spec` とする。WYSIWYG Editor UIはAdlaire-Design採用、Editor実装はAuteur内製とし、WYSIWYG Editor UI仕様と実装境界は `Docs/Master_Spec` に統合する。リポジトリ内の主要ファイルと管理対象は `Docs/Document_Index`、変更履歴は `Docs/Change_History` を参照する。
